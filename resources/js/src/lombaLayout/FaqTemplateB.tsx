...
export default function FaqTemplateB({ kompetisiDetail, sections, primaryColor }: any) {
    const theme = {
        bg: "bg-[#0a0a0a]",
        heroBg: "bg-[#0a0a0a]",
        sectionBg: "bg-[#0a0a0a]",
        cardContainer: "bg-[#111] border-t-2 border-gray-800",
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10 w-full max-w-7xl">
                    <div className="text-center space-y-4 sm:space-y-6 md:space-y-8">
                        <div className="hidden lg:inline-block group">
                            <span className="text-red-500 border-red-500 font-plex font-semibold text-xs sm:text-sm uppercase tracking-[0.2em] border-l-4 pl-3 sm:pl-4 md:pl-6 relative">
                                SUPPORT CENTER
                        ...
                        <p className={`text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl font-plex ${theme.textBody} max-w-4xl mx-auto leading-relaxed font-light px-2 sm:px-4`}>
                            Find answers to the most common questions about {kompetisiDetail?.nama_event || "the event"}.
                        </p>
                    </div>
                </div>
            </section>
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10 w-full max-w-7xl">
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
                            ...
                            <div className="lg:col-span-3">
                                <div className={`w-full flex flex-col ${theme.cardContainer} backdrop-blur-md shadow-2xl rounded-xl overflow-hidden`}>
                                    {section.questions.map((faq: any, questionIndex: number) => (
                                        <div key={questionIndex} className={`border-b ${theme.divider} last:border-b-0 hover:bg-white/[0.02] transition-all duration-300`}>
                                            <FaqCard question={faq.question} answer={faq.answer} isModern={true} primaryColor={primaryColor} />
                                        </div>
                                    ))}
                                </div>
                            </div>
